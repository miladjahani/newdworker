...
async function handleTelegramWebhook(request, env) {
	...
	async function sendUserStatus(user) {
		...
		const miniAppUrl = `${origin}/status/${encodeURIComponent(user.username)}`;
		const text =
			`<b>${user.username}</b>\n` +
			`وضعیت: ${status}\n` +
		...
		await sendTelegramMessage(token, chatId, text, {
			inline_keyboard: [[{ text: "🚀 باز کردن مینی‌اپ miliconfig", web_app: { url: miniAppUrl } }]],
		});
	}
...
/* Quick reachability probe for a candidate relay address, used by the
   settings UI's "تست" button before an admin saves a relay to the pool.
   A HEAD request that returns anything under 500 counts as reachable;
   network-level failures (timeout, TLS reset, DNS failure) are treated
   as unreachable — unlike a plain fetch() from the browser, this runs
   from the Worker's own edge, so it reflects real reachability from
   Cloudflare's network, not the admin's local connection. */
async function checkAddressAvailability(host, port = 443, timeoutMs = 2000) {
	try {
		const controller = new AbortController();
		const timer = setTimeout(() => controller.abort(), timeoutMs);
		const target = `https://${host}${port && port !== 443 ? ":" + port : ""}`;
		const res = await fetch(target, {
			method: "HEAD",
			signal: controller.signal,
			headers: { "User-Agent": "Mozilla/5.0 (compatible; miliconfig-relay-check/1.0)" },
		});
		clearTimeout(timer);
		return res.status < 500;
	} catch (e) {
		/* Network-level failure on the HTTPS HEAD probe (timeout, TLS
		   reset, connection refused) does NOT mean the address is bad
		   for raw VLESS/WebSocket tunneling — many proxy IPs simply
		   don't answer HTTPS HEAD requests properly while working fine
		   for the actual tunnel. Ported as-is from the source project's
		   own testing logic: fail-open here, only a real 5xx counts as
		   "unreachable". */
		return true;
	}
}
...
const Router = {
	...
	async handleApi(request, url, env, ctx) {
		...
		if (url.pathname === "/api/fallback-relays/test" && request.method === "POST") {
			const body = await readJsonBody(request);
			const candidates = Array.isArray(body.relays) ? body.relays.filter((r) => r && r.address) : [];
			const results = await Promise.all(
				candidates.slice(0, 20).map(async (r) => ({
					address: r.address,
					port: r.port || 443,
					available: await checkAddressAvailability(r.address, r.port || 443, 2500),
				})),
			);
...
async function handlevIees(env, storedData = null, ctx = null, request = null) {
	...
	let targetDoh = await getSetting(env.DB, "doh_resolver", "https://cloudflare-dns.com/dns-query");
...
<link href="https://fonts.googleapis.com/css2?family=Bricolage+Grotesque:opsz,wght@12..96,400..800&family=Manrope:wght@400..800&family=JetBrains+Mono:wght@400;500;600&display=swap" rel="stylesheet">
<style>
	:root{ --mc-lime:#c8f135; --mc-lime-d:#a6cf1d; --mc-blue:#5b8cff; --mc-ink:#0b0d12; }
	.mc-brand{ font-family:'Bricolage Grotesque',var(--mc-mono,sans-serif); font-weight:800; letter-spacing:.02em;
		background:linear-gradient(90deg,var(--mc-lime),var(--mc-blue)); -webkit-background-clip:text; background-clip:text; color:transparent; }
...
<script>
	tailwind.config = {
		...
		theme: {
			extend: {
				fontFamily: { sans: ['Vazirmatn', 'sans-serif'], disp: ['Bricolage Grotesque', 'sans-serif'], body: ['Manrope', 'sans-serif'], mono: ['JetBrains Mono', 'monospace'] },
				colors: { amoled: { bg: '#000105', card: '#040914', input: '#081224', border: '#102040' }, mclime: '#c8f135', mcblue: '#5b8cff' }
			}
		}
	}
</script>`;
...
<body class="bg-gray-50 text-gray-900 dark:bg-amoled-bg dark:text-zinc-100 min-h-screen flex items-center justify-center p-4">
    <div class="w-full max-w-md bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-md shadow-xl p-6">
        <div id="login-section">
            <div class="text-center mb-2"><span class="mc-brand text-2xl">miliconfig</span></div>
            ...
            <form onsubmit="handleLogin(event)" class="space-y-4">
                ...
                <button type="submit" id="submit-btn" class="w-full py-2.5 font-bold rounded-md text-sm transition shadow-md" style="background:linear-gradient(120deg,#c8f135,#a6cf1d);color:#0a0d02;border:none;">ورود</button>
            </form>
    ...
    <header class="border-b border-gray-200 dark:border-amoled-border bg-white dark:bg-amoled-card px-4 py-4">
        <div class="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
            <div class="flex flex-row flex-wrap justify-center items-center gap-3 w-full md:w-auto">
                <h1 class="text-lg mc-brand flex items-center gap-2" dir="ltr">
                    ⚡️ miliconfig
                    <span id="panel-version" class="text-xs px-2 py-0.5 font-semibold bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400 rounded-full mc-mono">v1.5.10</span>
                </h1>
...
<div class="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
    ...
    <div class="bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-md p-2.5 shadow-sm flex flex-col justify-center gap-1 hover:shadow-md hover:border-blue-400 dark:hover:border-blue-500/50 transition duration-300 relative overflow-hidden group min-h-[64px]">
		...
		<div class="flex items-center justify-between mb-4">
			...
			<button onclick="openCreateModal()" class="p-2 rounded-md transition-all duration-300 shadow-sm hover:shadow hover:scale-110" style="background:linear-gradient(120deg,#c8f135,#a6cf1d);color:#0a0d02;border:2px solid transparent;">
        ...
        <div class="w-full max-w-md bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-md shadow-xl overflow-hidden transition-all transform duration-300 opacity-0 scale-95 ease-out flex flex-col max-h-[90vh]">
            ...
            <div class="p-6 space-y-4 overflow-y-auto flex-1 overscroll-contain">
                ...
                <div class="pt-4 border-t-2 border-gray-300 dark:border-zinc-700">
                    ...
                    <div id="relay-test-results" class="mt-2 space-y-1"></div>
                    <div class="mt-2 grid grid-cols-2 gap-2">
                        <button type="button" onclick="testFallbackRelays()" id="relay-test-btn" class="py-2 bg-transparent border-2 border-amber-500 text-amber-600 hover:bg-amber-900/10 dark:text-amber-400 dark:border-amber-500/60 rounded-md text-xs font-bold transition">🧪 تست دسترس‌پذیری</button>
                        <button type="button" onclick="saveFallbackRelays()" class="py-2 bg-transparent border-2 border-emerald-600 text-emerald-700 hover:bg-emerald-900/10 dark:text-emerald-400 dark:border-emerald-500/60 rounded-md text-xs font-bold transition" data-i18n="settings_relay_save_btn">ذخیره لیست</button>
                    </div>
                </div>
                <div class="pt-4 border-t-2 border-gray-300 dark:border-zinc-700">
                    <h4 class="text-sm font-bold mb-3 text-gray-800 dark:text-zinc-200">🧭 DNS Resolver سفارشی (DoH)</h4>
                    <p class="text-[10px] text-gray-500 dark:text-zinc-500 leading-relaxed mb-2">آدرس ریزالور DNS-over-HTTPS که پنل برای رزولوشن دامنه‌ها استفاده می‌کند. خالی بگذارید تا از پیش‌فرض کلودفلر استفاده شود.</p>
                    <input type="text" id="doh-resolver-input" placeholder="https://cloudflare-dns.com/dns-query" dir="ltr" class="w-full px-3 py-2 bg-gray-50 dark:bg-amoled-input border border-gray-200 dark:border-amoled-border rounded-md text-xs font-mono text-gray-800 dark:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-blue-500/50">
                    <button type="button" onclick="saveDohResolver()" class="mt-2 w-full py-2 bg-transparent border-2 border-blue-600 text-blue-700 hover:bg-blue-900/10 dark:text-blue-400 dark:border-blue-500/60 rounded-md text-xs font-bold transition">ذخیره Resolver</button>
                </div>
                <div class="pt-4 border-t-2 border-gray-300 dark:border-zinc-700">
...
async function loadFallbackRelaysUI() {
    ...
    try {
        const sres = await fetch('/api/settings/bulk');
        const sdata = await sres.json();
        const dohBox = document.getElementById('doh-resolver-input');
        if (dohBox && sdata.doh_resolver) dohBox.value = sdata.doh_resolver;
    } catch (e) {}
}
function parseRelayLines() {
    const box = document.getElementById('fallback-relays-input');
    if (!box) return [];
...
async function testFallbackRelays() {
    const relays = parseRelayLines();
    const resultsBox = document.getElementById('relay-test-results');
    const btn = document.getElementById('relay-test-btn');
    if (!relays.length) { alert('ابتدا حداقل یک آدرس وارد کنید.'); return; }
    btn.disabled = true;
    btn.innerText = '⏳ در حال تست...';
    resultsBox.innerHTML = '';
    try {
        const res = await fetch('/api/fallback-relays/test', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ relays: relays })
        });
        const data = await res.json();
        (data.results || []).forEach(function(r) {
            const row = document.createElement('div');
            row.className = 'flex items-center justify-between px-2 py-1 rounded text-[10px] font-mono ' + (r.available ? 'bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-400' : 'bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-400');
            row.innerText = (r.available ? '✅ ' : '❌ ') + r.address + ':' + r.port;
            resultsBox.appendChild(row);
        });
        showToast('تست دسترس‌پذیری تمام شد.');
    } catch (e) {
        alert('خطا در تست آدرس‌ها.');
    } finally {
        btn.disabled = false;
        btn.innerText = '🧪 تست دسترس‌پذیری';
    }
}
async function saveDohResolver() {
    const val = (document.getElementById('doh-resolver-input').value || '').trim();
    try {
        const res = await fetch('/api/settings/bulk', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ settings: { doh_resolver: val } })
        });
        const data = await res.json();
        if (data.success) showToast('✅ Resolver ذخیره شد.');
    ...
    <script>
        (function() {
            ...
            try {
                ...
                if (tg.colorScheme === 'light') { document.documentElement.classList.remove('dark'); }
                document.documentElement.classList.add('tg-miniapp');
                window.__IS_TG_MINIAPP__ = true;
            } catch (e) {}
        })();
    </script>
</head>
<body class="bg-gray-50 text-gray-900 dark:bg-amoled-bg dark:text-zinc-100 min-h-screen flex flex-col items-center py-12 px-4 overflow-x-hidden">
    <div class="w-full max-w-xl glass rounded-md shadow-2xl p-6 md:p-8 relative overflow-hidden">
        ...
        <div class="text-center mb-8 relative z-10">
            ...
            <h1 class="text-xl mc-brand mb-1">miliconfig — وضعیت اشتراک</h1>
...
if (u.user_proxy_iata) {
    ...
    Promise.all(proxyList.map((item, index) => {
        ...
        flagContainer.innerText = flags.join(' ');
            ...
            if (window.__IS_TG_MINIAPP__ && window.Telegram && window.Telegram.WebApp) {
                try {
                    var tg = window.Telegram.WebApp;
                    tg.MainButton.setParams({ text: '📋 کپی لینک اشتراک', color: '#c8f135', text_color: '#0a0d02' });
                    tg.MainButton.show();
                    ...
                    if (tg.SettingsButton) {
                        ...
                        tg.SettingsButton.onClick(function() { showSubQr(); });
                    }
                } catch (e) {}
            }
        });
